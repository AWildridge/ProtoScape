...
public final class LoginDecoder extends StatefulFrameDecoder<LoginDecoderState> {
    ...
    private Object decodePayload(ChannelHandlerContext ctx, Channel channel,
        ...
        if (buffer.readableBytes() >= loginLength) {
             ...
             * RSA
             * 
             * ChannelBuffer securePayload = decryptRsa(payload, securePayloadLength);
             *
             * private ChannelBuffer decryptRsa(ChannelBuffer buffer, int length) {
             * byte[] bytes = new byte[length];
             * buffer.readBytes(bytes);
             * byte[] decBytes = new BigInteger(bytes).modPow(exponent, modulus);
             * return ChannelBuffers.wrappedBuffer(decBytes);
             * }
             * 
             */
