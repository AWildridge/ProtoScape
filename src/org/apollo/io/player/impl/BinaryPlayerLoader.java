...
public final class BinaryPlayerLoader implements PlayerLoader {
    ...
    public PlayerLoaderResponse loadPlayer(PlayerCredentials credentials) throws Exception {
        ...
        try {
            ...
            if (World.getWorld().isPlayerOnline(credentials.getUsername())) {
                return new PlayerLoaderResponse(LoginConstants.STATUS_ACCOUNT_ONLINE);
            }
            ...
            size = in.readUnsignedByte();
            //size = 0;
            Friend friends = p.getFriends();
            for (int i = 0; i < size; i++) {
                String user = StreamUtil.readString(in);
                int event = in.readUnsignedByte();
                friends.add(user, friends.getValue(event), true);
            }
            
            if(in.available() > 0) {
	            if (in.readBoolean()) {
	                p.setPinSet(true);
	                p.setBankPin(new BankPin(p));
	                p.getBankPin().setBankPin(in.readUTF());
	                System.out.println("Setting bank pin.");
	            }
            }
