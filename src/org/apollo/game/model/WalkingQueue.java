...
import org.apollo.game.scheduling.impl.EnergyRestorationTask;
...
public final class WalkingQueue {
    ...
    public void pulse() {
        ...
        if (next != null) {
            ...
            if (character.getEventManager() != null) {
                ...
                if (player.isRunning() && player.getRunEnergy() > 0) {
                    next = points.poll();
                    if (next != null) {
	...
	if(player.isRunning())
	    player.setRunEnergy(player.getRunEnergy() - 1.00);
	else
	    player.setRunEnergy(player.getRunEnergy() - .40);
        if(!player.isRestoreEnergy()) {
            World.getWorld().schedule(new EnergyRestorationTask(player));
            player.setRestoreEnergy(true);
        }
    }
    /**
